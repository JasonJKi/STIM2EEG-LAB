...
#### About LSL
The remarkable advantage of using LSL is that it uses tcp/ip protocol standards allowing multiple streams to be transmitted and read over the local area network. This means that data can be trasmitted in real time without boggling up any one paticular device's memory or cpu. All devices connected over the LAN can receive and transmit data using the LSL. This decentralized system allows for any system to push and pull packet(s) of data from the LAN.
